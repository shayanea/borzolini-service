...
export class FileUploadController {
  ...
  async listFiles(
    ...
    try {
      const files = await this.localStorageService.listFiles(
        category,
        subcategory,
      );
  ...
  async cleanupOldFiles(@Body() body: { daysOld?: number }): Promise<{
    success: boolean;
    message: string;
    deletedCount: number;
  }> {
    try {
      ...
      const deletedCount =
        await this.localStorageService.cleanupOldFiles(daysOld);
