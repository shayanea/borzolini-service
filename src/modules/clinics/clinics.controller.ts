import { Controller, Get, Post, Body, Patch, Param, Delete, Query, UseGuards, Request, HttpStatus, HttpCode } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiBearerAuth, ApiQuery, ApiParam } from '@nestjs/swagger';
import { ClinicsService, ClinicFilters, ClinicSearchOptions } from './clinics.service';
...
export class ClinicsController {
  ...
  @ApiQuery({ name: 'name', required: false, description: 'Filter by clinic name' })
  @ApiQuery({ name: 'city', required: false, description: 'Filter by city' })
  @ApiQuery({ name: 'state', required: false, description: 'Filter by state' })
  @ApiQuery({ name: 'is_verified', required: false, description: 'Filter by verification status' })
  ...
  @ApiQuery({ name: 'specializations', required: false, description: 'Filter by specializations' })
  @ApiQuery({ name: 'rating_min', required: false, description: 'Minimum rating filter' })
  @ApiQuery({ name: 'rating_max', required: false, description: 'Maximum rating filter' })
  @ApiQuery({ name: 'page', required: false, description: 'Page number', example: 1 })
  @ApiQuery({ name: 'limit', required: false, description: 'Items per page', example: 10 })
  @ApiQuery({ name: 'sort_by', required: false, description: 'Sort field', example: 'rating' })
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Admin role required' })
  @ApiResponse({ status: 404, description: 'Clinic not found' })
  @ApiResponse({ status: 409, description: 'Conflict - Clinic name already exists' })
  async update(@Param('id') id: string, @Body() updateClinicDto: UpdateClinicDto): Promise<Clinic> {
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Admin role required' })
  @ApiResponse({ status: 404, description: 'Clinic not found' })
  @ApiResponse({ status: 409, description: 'Conflict - User already staff member' })
  async addStaff(@Param('id') clinicId: string, @Body() createClinicStaffDto: CreateClinicStaffDto): Promise<ClinicStaff> {
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async removeStaff(@Param('clinicId') clinicId: string, @Param('userId') userId: string): Promise<void> {
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Admin role required' })
  @ApiResponse({ status: 404, description: 'Clinic not found' })
  @ApiResponse({ status: 409, description: 'Conflict - Service already exists' })
  async addService(@Param('id') clinicId: string, @Body() createClinicServiceDto: CreateClinicServiceDto): Promise<ClinicService> {
  ...
  async updateService(@Param('id') id: string, @Body() updateServiceDto: Partial<CreateClinicServiceDto>): Promise<ClinicService> {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Clinic not found' })
  @ApiResponse({ status: 409, description: 'Conflict - User already reviewed this clinic' })
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - Admin role required' })
  @ApiResponse({ status: 404, description: 'Clinic not found' })
  async addPhoto(@Param('id') clinicId: string, @Body() photoData: { photo_url: string; caption?: string; category?: string; is_primary?: boolean }): Promise<ClinicPhoto> {
