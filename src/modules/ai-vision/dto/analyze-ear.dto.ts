...
export class AnalyzeEarDto {
  @ApiProperty({ description: 'Pet species', enum: ['cat', 'dog'], example: 'dog' })
  @IsEnum(['cat', 'dog'])
  species!: 'cat' | 'dog';

  @ApiPropertyOptional({ description: 'Pet breed', example: 'Labrador Retriever' })
  @IsOptional()
  @IsString()
  breed?: string;

  @ApiPropertyOptional({ description: 'Observed symptoms', example: 'Head shaking, discharge' })
  @IsOptional()
  @IsString()
  symptoms?: string;
