...
import { AiHealthInsight, InsightCategory } from './entities/ai-health-insight.entity';
...
export class AiHealthController {
  ...
  async generateRecommendations(@Body() dto: GenerateRecommendationsDto): Promise<AiHealthInsight[]> {
    return await this.aiHealthService.generateRecommendations(dto);
  }

  @Get('recommendations/combined')
  @ApiOperation({
    summary: "Get up to 5 combined AI recommendations across all user's pets",
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Pet not found' })
  async getPetInsights(@Param('petId') petId: string, @Query('includeDismissed') includeDismissed: boolean = false): Promise<AiHealthInsight[]> {
  ...
  async getInsightsByCategory(@Param('petId') petId: string, @Param('category') category: InsightCategory): Promise<AiHealthInsight[]> {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Pet not found' })
  async getUrgentInsights(@Param('petId') petId: string): Promise<AiHealthInsight[]> {
  ...
  async updateInsight(@Param('insightId') insightId: string, @Body() updateDto: UpdateInsightDto): Promise<AiHealthInsight> {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Pet not found' })
  async refreshInsights(@Param('petId') petId: string): Promise<AiHealthInsight[]> {
  ...
  async getAiHealthDashboard(@Param('petId') petId: string) {
    const [insightsSummary, urgentInsights, recentRecommendations] = await Promise.all([
      this.aiHealthService.getInsightsSummary(petId),
      this.aiHealthService.getUrgentInsights(petId),
      this.aiHealthService.getInsightsByCategory(petId, InsightCategory.HEALTH),
    ]);

    // Calculate health score based on insights
    const healthScore = this.calculateHealthScore(insightsSummary, urgentInsights);

    // Generate next actions
    const nextActions = this.generateNextActions(urgentInsights, recentRecommendations);
   ...
   */
  private calculateHealthScore(summary: any, urgentInsights: AiHealthInsight[]): number {
    let score = 100;
   ...
   */
  private generateNextActions(urgentInsights: AiHealthInsight[], recentRecommendations: AiHealthInsight[]): string[] {
    ...
    recentRecommendations.slice(0, 3).forEach((insight) => {
      if (insight.suggested_action && !actions.includes(insight.suggested_action)) {
