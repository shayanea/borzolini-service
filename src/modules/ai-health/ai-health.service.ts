...
import { AiHealthInsight, InsightType, InsightCategory, UrgencyLevel } from './entities/ai-health-insight.entity';
...
export class AiHealthService {
  ...
  async getInsightsSummary(petId: string): Promise<{
    ...
    const [total, urgent, recommendations, alerts, recent] = await Promise.all([
      this.aiInsightRepository.count({ where: { pet_id: petId, dismissed: false } }),
