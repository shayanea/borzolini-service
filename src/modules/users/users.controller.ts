...
import { RequestPhoneVerificationDto, VerifyPhoneDto, ResendPhoneVerificationDto, PhoneVerificationStatusDto } from './dto/phone-verification.dto';
...
export class UsersController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Recalculate current user profile completion percentage' })
  @ApiResponse({ status: 200, description: 'Profile completion recalculated successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Recalculate profile completion percentage for a specific user (Admin only)' })
  @ApiResponse({ status: 200, description: 'Profile completion recalculated successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required' })
  @ApiResponse({ status: 404, description: 'User not found' })
  async recalculateUserProfileCompletion(@Param('id') id: string, @Request() req: AuthenticatedRequest) {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Recalculate profile completion for all users (Admin only)' })
  @ApiResponse({ status: 200, description: 'All profile completions recalculated successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required' })
  ...
  @ApiResponse({ status: 400, description: 'Bad request or phone already verified' })
  @ApiResponse({ status: 404, description: 'User not found' })
  ...
  @ApiResponse({ status: 400, description: 'Bad request or too frequent requests' })
  @ApiResponse({ status: 404, description: 'User not found' })
  ...
  @ApiResponse({ status: 200, description: 'Status retrieved successfully', type: PhoneVerificationStatusDto })
  @ApiResponse({ status: 404, description: 'User not found' })
