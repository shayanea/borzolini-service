...
export class UsersSeeder {
    ...
    try {
      for (const userData of users) {
        ...
        const profileCompletionPercentage =
          await this.calculateProfileCompletion(savedUser);
        await this.userRepository.update(savedUser.id, {
          profileCompletionPercentage,
        });

        this.logger.log(
          `Created user: ${savedUser.email} (${savedUser.role}) - Profile completion: ${profileCompletionPercentage}%`,
        );
      }
  ...
  private async createUserPreferences(user: User): Promise<void> {
    const preferences = this.userPreferencesRepository.create({
      ...
      privacySettings: {
        profileVisibility:
          user.role === UserRole.VETERINARIAN ? "public" : "public",
        showPhone:
          user.role === UserRole.VETERINARIAN || user.role === UserRole.STAFF,
        showAddress:
          user.role === UserRole.VETERINARIAN || user.role === UserRole.STAFF,
        showEmail:
          user.role === UserRole.VETERINARIAN || user.role === UserRole.STAFF,
