import { Controller, Get, Post, Body, Patch, Param, Delete, Query, UseGuards, Request, ParseIntPipe, ParseEnumPipe } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiBearerAuth, ApiQuery, ApiParam } from '@nestjs/swagger';

import { AppointmentsService, AppointmentFilters, AppointmentStats, TimeSlot } from './appointments.service';
...
export class AppointmentsController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create a new appointment', description: 'Create a new appointment for the authenticated user' })
  @ApiResponse({ status: 201, description: 'Appointment created successfully', type: Appointment })
  @ApiResponse({ status: 400, description: 'Bad request - invalid appointment data' })
  ...
  @Get()
  @ApiOperation({ summary: 'Get all appointments', description: 'Retrieve all appointments with optional filtering and pagination' })
  ...
  @ApiQuery({ name: 'type', required: false, enum: AppointmentType, description: 'Filter by appointment type' })
  @ApiQuery({ name: 'priority', required: false, enum: AppointmentPriority, description: 'Filter by appointment priority' })
  @ApiQuery({ name: 'clinic_id', required: false, type: String, description: 'Filter by clinic ID' })
  @ApiQuery({ name: 'staff_id', required: false, type: String, description: 'Filter by staff ID' })
  @ApiQuery({ name: 'pet_id', required: false, type: String, description: 'Filter by pet ID' })
  @ApiQuery({ name: 'owner_id', required: false, type: String, description: 'Filter by owner ID (admin only)' })
  @ApiQuery({ name: 'date_from', required: false, type: String, description: 'Filter by start date (ISO string)' })
  @ApiQuery({ name: 'date_to', required: false, type: String, description: 'Filter by end date (ISO string)' })
  @ApiQuery({ name: 'is_telemedicine', required: false, type: Boolean, description: 'Filter by telemedicine appointments' })
  @ApiQuery({ name: 'is_home_visit', required: false, type: Boolean, description: 'Filter by home visit appointments' })
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search in pet name, owner name, or notes' })
  async findAll(
    @Request() req: any,
  ...
  @Get('my-appointments')
  @ApiOperation({ summary: 'Get current user appointments', description: 'Retrieve all appointments owned by the authenticated user' })
  ...
  @Get('pet/:petId')
  @ApiOperation({ summary: 'Get appointments by pet', description: 'Retrieve all appointments for a specific pet' })
  @ApiResponse({ status: 200, description: 'Pet appointments retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @Get('clinic/:clinicId')
  @ApiOperation({ summary: 'Get appointments by clinic', description: 'Retrieve all appointments for a specific clinic' })
  ...
  @ApiQuery({ name: 'date', required: false, type: String, description: 'Filter by specific date (ISO string)' })
  ...
  @Get('staff/:staffId')
  @ApiOperation({ summary: 'Get appointments by staff member', description: 'Retrieve all appointments for a specific staff member' })
  ...
  @Get('stats')
  @ApiOperation({ summary: 'Get appointment statistics', description: 'Retrieve comprehensive statistics about all appointments' })
  ...
  @ApiOperation({ summary: 'Get available time slots', description: 'Get available time slots for a specific clinic and date' })
  @ApiResponse({ status: 200, description: 'Available time slots retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiParam({ name: 'clinicId', description: 'Clinic ID' })
  @ApiQuery({ name: 'date', required: true, type: String, description: 'Date to check (ISO string)' })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get appointment by ID', description: 'Retrieve a specific appointment by its ID' })
  ...
  async findOne(@Param('id') id: string, @Request() req: any): Promise<Appointment> {
    ...
    if (appointment.owner_id !== req.user.id && 
        ![UserRole.ADMIN, UserRole.VETERINARIAN, UserRole.STAFF].includes(req.user.role)) {
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Update appointment', description: 'Update a specific appointment by its ID' })
  ...
  @Patch(':id/status')
  @ApiOperation({ summary: 'Update appointment status', description: 'Update the status of a specific appointment' })
  ...
  @ApiResponse({ status: 404, description: 'Appointment not found' })
  @ApiParam({ name: 'id', description: 'Appointment ID' })
  @ApiQuery({ name: 'status', required: true, enum: AppointmentStatus, description: 'New appointment status' })
  async updateStatus(
    @Param('id') id: string,
    @Query('status', new ParseEnumPipe(AppointmentStatus)) status: AppointmentStatus,
    @Request() req: any,
  ): Promise<Appointment> {
  ...
  @Patch(':id/reschedule')
  @ApiOperation({ summary: 'Reschedule appointment', description: 'Reschedule a specific appointment to a new date/time' })
  @ApiResponse({ status: 200, description: 'Appointment rescheduled successfully', type: Appointment })
  @ApiResponse({ status: 400, description: 'Bad request - invalid new date' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Appointment not found' })
  @ApiResponse({ status: 409, description: 'New time conflicts with existing appointment' })
  @ApiParam({ name: 'id', description: 'Appointment ID' })
  @ApiQuery({ name: 'new_date', required: true, type: String, description: 'New appointment date (ISO string)' })
  ...
  @Delete(':id')
  @ApiOperation({ summary: 'Cancel appointment', description: 'Cancel a specific appointment by its ID' })
  ...
  @Get('today')
  @ApiOperation({ summary: 'Get today appointments', description: 'Get all appointments scheduled for today' })
  ...
  @Get('upcoming')
  @ApiOperation({ summary: 'Get upcoming appointments', description: 'Get all upcoming appointments' })
